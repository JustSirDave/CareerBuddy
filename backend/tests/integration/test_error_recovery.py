...
class TestRenderFailure:
    ...
    async def test_render_failed_status_used(self, db_session, test_user, sample_resume_data):
        ...
        job = Job(
            ...
            type="resume",
            status="collecting",
            answers={**sample_resume_data, "_step": "finalize"},
        )
        db_session.add(job)
        db_session.commit()
        db_session.refresh(job)

        with patch("app.services.conversation_router.renderer.render_resume", side_effect=RuntimeError("Render failed")):
            with patch("app.services.conversation_router.handle_error", new_callable=AsyncMock) as mock_err:
                reply = await handle_resume(db_session, job, "retry")
    ...
    @pytest.mark.skip(
        reason="Render retry path changed in conversation_router. Needs realignment with current RENDER_FAILED handling — see SDD Section 6.3."
    )
...
class TestDropoutRecovery:
    ...
    async def test_help_mid_flow_shows_step_reminder(self, db_session, test_user, test_job):
        ...
        from app.services.conversation_router import handle_inbound, STEP_LABELS
