...
class TestCompleteResumeFlow:
    ...
    @pytest.mark.skip(
        reason="Flow hits real OpenAI without mock; flaky under quota limits. Rewrite with mocked ai.generate_document — see SRS NFR-051."
    )
...
class TestCompleteCVFlow:
    ...
    @pytest.mark.skip(
        reason="Flow hits real OpenAI without mock; job=None after 429. Rewrite with mocked ai.generate_document — see SRS NFR-051."
    )
...
class TestDocumentGenerationIntegration:
    ...
    @pytest.mark.skip(
        reason="Renderer text extraction asserts table body not in plain text output. Rewrite after renderer stabilises — see SDD Section 3.7."
    )
...
class TestDatabaseIntegration:
    ...
    def test_message_tracking(self, db_session, test_job):
        ...
        messages = [
            Message(user_id=test_job.user_id, job_id=test_job.id, direction="inbound", content="Hello"),
            Message(user_id=test_job.user_id, job_id=test_job.id, direction="outbound", content="Hi"),
            Message(user_id=test_job.user_id, job_id=test_job.id, direction="inbound", content="Resume please"),
        ]
        for msg in messages:
            db_session.add(msg)
        db_session.commit()
...
class TestErrorRecovery:
    ...
    async def test_invalid_input_recovery(self, db_session, test_user, test_job):
        ...
        response = await handle_resume(db_session, test_job, "invalid")
